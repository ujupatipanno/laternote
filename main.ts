...
export default class TrashNotePlugin extends Plugin {
	...
	async onload() {
		...
		// 파일 rename 이벤트 감지 - 잡동사니 노트 경로 자동 업데이트
		this.registerEvent(
			this.app.vault.on('rename', async (file, oldPath) => {
				if (this.settings.trashNotePath === oldPath) {
					this.settings.trashNotePath = file.path;
					await this.saveSettings();
					new Notice(`잡동사니 노트의 이름이 변경되었습니다: "${file.name}"`);
				}
			})
		);

		// 명령어 등록 (이따보기 컨셉에 맞춘 라벨)
		const commands: Array<[string, string, string, boolean?, string?]> = [
			['set-trash-note', '현재 문서를 잡동사니 노트로 지정', 'setTrashNote'],
			['move-selection-to-trash', '잡동사니 노트로 옮기기', 'moveSelectionToTrash', true, 'lucide-brush-cleaning'],
			['open-trash-note', '잡동사니 노트 열기', 'openTrashNote', false, 'lucide-tool-case'],
			['move-trash-to-new-note', '잡동사니 노트 내용을 기록 노트로 옮기고 비우기', 'moveTrashToNewNote'],
		];
		...
		// 리본에 아이콘 추가
		this.addRibbonIcon('lucide-brush-cleaning', '잡동사니 노트로 옮기기', async () => {
			const view = this.app.workspace.getActiveViewOfType(MarkdownView);
			if (view?.editor) {
				await this.moveSelectionToTrash(view.editor);
			} else {
				new Notice('에디터를 찾을 수 없습니다.');
			}
		});

		this.addRibbonIcon('lucide-tool-case', '잡동사니 노트 열기', async () => {
	...
	private async setTrashNote() {
		...
		this.settings.trashNotePath = file.path;
		await this.saveSettings();
		new Notice(`잡동사니 노트가 "${file.basename}"으로 지정되었습니다.`);
	}
	...
	private async moveSelectionToTrash(editor: Editor) {
		...
		if (currentFile?.path === trashFile.path) {
			new Notice('잡동사니 노트에서는 이 명령을 사용할 수 없습니다.');
			return;
		}
		...
		// 옮길 내용이 없는 경우 알림만 띄우고 종료
		if (!content || content.trim() === '') {
			new Notice('옮길 내용이 없습니다.');
			return;
		}
	...
	private async appendToTrashNote(content: string) {
		this.appendQueue = this.appendQueue.then(async () => {
			try {
				...
				console.error(error);
				new Notice('잡동사니 노트로 옮기는 중 오류가 발생했습니다.');
			}
		});
	...
	private async openTrashNote() {
		try {
			...
			const existingLeaf = this.app.workspace.getLeavesOfType('markdown').find(
				(leaf) => leaf.view instanceof MarkdownView && leaf.view.file?.path === trashFile.path
			);
			...
			console.error(error);
			new Notice('잡동사니 노트를 여는 중 오류가 발생했습니다.');
		}
	}
	...
	private async moveTrashToNewNote() {
		...
		new CreateNoteModal(this.app, this, async (title: string, modal) => {
			...
			try {
				...
				new Notice(`"${safeTitle}" 기록 노트가 생성되었습니다.`);
				return true;
			} catch (error) {
				console.error(error);
				modal.showError('노트를 생성하거나 잡동사니 노트를 비우는 중 오류가 발생했습니다.');
				return false;
			}
		}).open();
	}
	...
	private async ensureTrashNoteFile(): Promise<TFile | null> {
		try {
			...
			await this.saveSettings();
			new Notice(`잡동사니 노트가 없어 기본 파일을 생성했습니다: ${created.path}`);
			return created;
		} catch (error) {
			console.error(error);
			new Notice('잡동사니 노트 기본 파일을 생성하는 중 오류가 발생했습니다.');
			return null;
		}
	}
...
class TrashNoteSuggestModal extends SuggestModal<TFile> {
	...
	constructor(app: App, plugin: TrashNotePlugin, settingTab?: TrashNoteSettingTab) {
		...
		this.setPlaceholder('잡동사니 노트로 지정할 문서를 검색하세요');
	}
	...
	async onChooseSuggestion(file: TFile, evt: MouseEvent | KeyboardEvent) {
		...
		if (this.settingTab?.trashNoteSetting) {
			const newDesc = file.path ? `현재 지정: ${file.path}` : '현재 지정된 잡동사니 노트가 없습니다.';
...
class CreateNoteModal extends Modal {
	...
	onOpen() {
		...
		contentEl.createEl('h3', { text: '기록 노트의 제목을 입력해주세요' });
...
class TrashNoteSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const trashDesc = this.plugin.settings.trashNotePath ? `현재 지정: ${this.plugin.settings.trashNotePath}` : '현재 지정된 잡동사니 노트가 없습니다.';
		...
		// 기록 노트 제목 placeholder 형식
		new Setting(containerEl)
			.setName('기록 노트 제목 placeholder')
		...
		new Setting(containerEl)
			.setName('기록 노트 생성 위치')
			.setDesc('잡동사니 노트 내용을 옮길 때 새 노트를 생성할 폴더')
